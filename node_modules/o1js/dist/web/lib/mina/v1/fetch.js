 ...
 * ```
 */
async function fetchEvents(accountInfo, graphqlEndpoint = networkConfig.archiveEndpoint, filterOptions = {}, headers) {
    if (!graphqlEndpoint)
    ...
    const { publicKey, tokenId } = accountInfo;
    let [response, error] = await makeGraphqlRequest(getEventsQuery(publicKey, tokenId ?? TokenId.toBase58(TokenId.default), filterOptions), graphqlEndpoint, networkConfig.archiveFallbackEndpoints, { headers: { ...networkConfig.archiveDefaultHeaders, ...headers } });
    if (error)
        throw Error(error.statusText);
 ...
 * ```
 */
async function fetchActions(accountInfo, graphqlEndpoint = networkConfig.archiveEndpoint, headers) {
    if (!graphqlEndpoint)
    ...
    let [response, error] = await makeGraphqlRequest(getActionsQuery(publicKey, actionStates, tokenId), graphqlEndpoint, networkConfig.archiveFallbackEndpoints, { headers: { ...networkConfig.archiveDefaultHeaders, ...headers } });
    ...
    if (error) {
        ...
        /* _excludeTransactionInfo= */ true), graphqlEndpoint, networkConfig.archiveFallbackEndpoints, { headers: { ...networkConfig.archiveDefaultHeaders, ...headers } });
        if (error)
